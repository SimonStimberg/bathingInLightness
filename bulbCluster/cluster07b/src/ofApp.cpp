...
//    detector = objDetector();
	...
	flock.setBounds(-boxSize*0.5, -boxSize*0.5, -boxSize*0.5,   boxSize*0.5, boxSize*0.5, boxSize*0.5);
...
//    }
    ...
    // ofLogNotice(ofToString(kinectPointCloud));


    // HAND KINECT POINTS TO PARTICLES AND COMPUTE PARTICLES
    // comment this out to bypass particles
...
//    }
    ...
    // uncomment this to hand the kinect points directly to the bulbs
    // vector <ofPoint> currentParticlePosition = kinectPointCloud;
    flock.clearAttrPts();
    ...
    vector <ofPoint> currentParticlePosition = flock.getPositions();
...
void ofApp::keyPressed(int key){
    ...
    if( key == 'n'){
        // Boid3d * myBoy = flock.get(10);
        // ofLogNotices(ofToString(myBoy->this));
    }
