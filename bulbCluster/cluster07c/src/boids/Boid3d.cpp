...
void Boid3d:: update(const float amount, int myId) {
...
float* Boid3d::flockfull(const float amount, float *vec) {
	...
	if (flockPtr->hasAttractionPoints()) {
		...
		int target = 0;
		if(flockPtr->attractionPoints.size() > id) {
			target = int( roundf( flockPtr->attractionPoints.size() / flockPtr->size() * id ) );
		} else {
			target = id % flockPtr->attractionPoints.size();
		}

		AttractionPoint3d * point = flockPtr->attractionPoints.at(target);
		float dx = point->x - x;
		float dy = point->y - y;
		float dz = point->z - z;
		float d = ABS(dx) + ABS(dy)+ ABS(dz);
