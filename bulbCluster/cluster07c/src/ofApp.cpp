...
void ofApp::update(){
    ...
    for(unsigned int i = 0; i < bulbCluster.size(); i++){
        ...
        newIntensity += bulbCluster[i].computeIntensity(kinectPointCloud, 0.8);
        newIntensity += bulbCluster[i].computeIntensity(flockPositions, 0.5);
        
        intensities.push_back(bulbCluster[i].updateIntensity(newIntensity));
    }
