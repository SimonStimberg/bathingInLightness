        ...
        for(int y = 0; y < h; y += step) {
            for(int x = 0; x < w; x += step) {
                if(kinect.getDistanceAt(x, y) > nearClip && kinect.getDistanceAt(x, y) < (nearClip + clippingDepth) ) {
                    ...
                    // newPoint.rotate(-40, 0, 0);
                    
                    // clip the the point if its outside the box
                    if (newPoint.x >= -worldSize*0.5 && newPoint.x <= worldSize*0.5  &&  newPoint.y >= -worldSize*0.5 && newPoint.y <= worldSize*0.5  &&  newPoint.z >= -worldSize*0.5 && newPoint.z <= worldSize*0.5) {
