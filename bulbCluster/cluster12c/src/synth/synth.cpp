...
void PolySynth::setVoiceLevels(vector <float> & levels) {
    if (voices.size() == levels.size()) {
        ...
        for( size_t i=0; i<voices.size(); ++i ){
            ...
            // float newLevel = ofMap(ofClamp(levels[i], 0., 1.), 0., 1., 0., 0.9);    // to avoid clipping don't use the whole range? - the clipping appears apparently in the delay
            float newLevel = ofClamp(levels[i], 0., 1.); 
...
void PolySynth::Voice::setup( PolySynth & ui, int v, int pitch){
    ...
    // OSC 1 - Pulse Wave
                            osc1.out_pulse() * dB(-6.0f) >> mix.in_A();         // reduce gain before mixing to avoid clipping/saturation artefacts (not sure if necessary)
                                                   pitch >> osc1.in_pitch();
    1.5f >> driftLFO1.out_random() >> ui.drift_amt.ch(v) >> osc1.in_pitch();   
        vibratoLfo.out_triangle() >> ui.vibratoAmt.ch(v) >> osc1.in_pitch();  

    // OSC 2 - Sine Wave
                             osc2.out_sine() * dB(-6.0f) >> mix.in_B();         // reduce gain before mixing to avoid clipping/saturation artefacts (not sure if necessary)
                                                   pitch >> osc2.in_pitch();
