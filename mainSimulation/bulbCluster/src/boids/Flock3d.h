...
public:
	...
	// scale the system by this amount:
	float scale; 
...
//		setBounds(0, 0, 700, 700);
	...
	void gatherPositions() {
		...
		for(int i=0; i<boids.size(); i++){
			Boid3d *b = boids[i];
			boidPositions[i].x = b->x/scale;
			boidPositions[i].y = b->y/scale;
			boidPositions[i].z = b->z/scale;

			// push_back(ofPoint(b->x/scale, b->y/scale, b->z/scale));
		}
	...
	Flock3d * addAttractionPoint(float x, float y, float z, float force,
								 float sensorDist) {
		AttractionPoint3d * ap = new AttractionPoint3d(x * scale, y * scale, z * scale, force, sensorDist);
	...
	void drawScaled() {
		for(int i=0; i<boids.size(); i++){
			...
			ofPushMatrix();
			ofTranslate(b->x/scale, b->y/scale, b->z/scale);
			ofRect(-5/scale,-5/scale,10/scale,10/scale);
			ofPopMatrix();
			
			float lm = 10.f/scale;
			ofLine(b->x/scale, b->y/scale, b->z/scale, b->x/scale + b->vx*lm, b->y/scale + b->vy*lm, b->z/scale + b->vz*lm);
		}
	}
